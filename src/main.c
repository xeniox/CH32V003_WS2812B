...
int main(void) {
  ...
  while(1){
    ...
    LED_RAINBOWS(0, 10, 10); // variables are channel, delay speed in mS and width (number of leds that cover one cycle)
    //LED_RAINBOW_CYCLE(0, 100); // rainbow cycle with channel and delay in ms
    //LED_THEATER_CHASE(0, 0, 255, 0, 100); // theater chase with channel, RGB and delay in ms
    //LED_COLOUR_WIPE(0, 0, 0, 255, 250); // colour wipe with channel, RGB and delay in ms
    //LED_RGB_FLASH(0, 500, 255); // RGB flash with channel, speed in ms and brightness 0-255
    ...
    //LED_PULSE(0, 255, 0, 255, 25); // pulse with channel, RGB and delay in ms
  }
  return 0;
}
